...
class Waypoints(object):
    ...
    def __init__(self, speed=0.28, accuracy=0.003726646, timeout=5):
        ...
        self.sawyer_arm.move_to_neutral(timeout=self.timeout, speed=self.joint_speed) 
    ...
    def record(self):
        ...
        # Set the Square button to stop the trajectory recording
        show_id = self.navigator_io.register_callback(self.stop_recording, 'right_button_square')
    ...
    def playback(self):
        ...
        for (waypoint, pos) in self.waypoints:
            self.sawyer_arm.move_to_joint_positions(waypoint, timeout=3, threshold=self.accuracy)
